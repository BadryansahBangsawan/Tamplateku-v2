...
export function AnimatedBlogContent({ children }: AnimatedBlogContentProps) {
  ...
  useGSAP(() => {
    ...
    // Animate prose elements with stagger
    const proseElements = contentRef.current.querySelectorAll(
      "h1, h2, h3, h4, h5, h6, p, ul, ol, blockquote, pre, code, img"
    );
    ...
    <article
      ...
      dangerouslySetInnerHTML={{
        __html: children
          ? children
              ?.toString()
              .replace(/<h([1-6])([^>]*)>(.*?)<\/h[1-6]>/gi, (_match, level, attrs, content) => {
                const id = content
